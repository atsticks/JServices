...
public class GlassfishResolver implements ServiceResolver {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(GlassfishResolver.class);
	...
	public <T> T resolve(Service serviceURI, Class<T> targetInterface) throws ServiceUnavailableException {
		...
		System.out.print("Locating glassfish JNDI on " + host + ":" + port + "  ... ");
		...
		try {
			if (context == null) {
				...
				env.put(Context.INITIAL_CONTEXT_FACTORY, "com.sun.enterprise.naming.SerialInitContextFactory"); // "weblogic.jndi.WLInitialContextFactory"
				env.put(Context.PROVIDER_URL, providerURL); // "t3://localhost:7001"
				try {
